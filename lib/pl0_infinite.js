  ...
  DefaultScanner.prototype.scan = function(text) {
    ...
    var emit = function(text) {
      if (separators.indexOf(text) !== -1) {
        self.callback({type: 'sep', value: text});
      } else if (keywords.indexOf(text) !== -1) {
        self.callback({type: 'keyword', value: text});
      } else if (numbers.indexOf(text.slice(0,1)) != -1) {
        self.callback({type: 'number', value: text});
      } else {
        self.callback({type: 'ident', value: text});
      }
    }

    text.split(" ").forEach(emit);
